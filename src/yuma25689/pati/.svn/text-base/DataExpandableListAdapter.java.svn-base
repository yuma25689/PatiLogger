...
public class DataExpandableListAdapter extends BaseExpandableListAdapter {
        ...
        public View getChildView(int groupPosition, int childPosition,
    		...
    		if( item != null ) {
    			TextView DataMenuName
    				= (TextView)view.findViewById(R.id.toptext);
    			DataMenuName.setText(item.getMenuString());
    			TextView right
					= (TextView)view.findViewById(R.id.toptextright);
    			right.setText(item.getRightString());    			
    			if( item.getResId() != 0 ) {
    				ImageView DataMenuImg 
    				= (ImageView)view.findViewById(R.id.lsticon);
    				DataMenuImg.setImageResource(item.getResId());
    			}
    		}
            return view;  
        }
