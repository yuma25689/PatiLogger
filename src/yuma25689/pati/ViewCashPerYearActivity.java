...
public class ViewCashPerYearActivity extends Activity 
	implements AdapterView.OnItemClickListener {

	private ActivityCommonLogic commLogic = ActivityCommonLogicFactory.create();
	//new ActivityCommonLogic();
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.view_per_year);

        setTitle(getString(R.string.DlgTitle_ViewPerYear));
        commLogic.init(this);

        listView = (ListView) this.findViewById(R.id.lstViewParYearMain);
    ...
    private void updateAdapter() {
    	...
    	// queryでSELECTを実行
    	String[] columns = {"sum(CashFlow)","SUBSTR(WorkDate,1,4) AS WorkYear", "sum(ExpVal)"};
    	String selection = null;
    	String groupBy = "SUBSTR(WorkDate,1,4)";
    	String orderBy = "SUBSTR(WorkDate,1,4) desc";
    	...
    	try {
	    	...
	    	for (int i = 0; i < item.length; i++) {
	    		//ids[i] = c.getInt(0);
	    	    item[i] = TableControler.getFmtYear( c.getString(1) ); 
	    		...
	    		Cursor c3 = dbRead.rawQuery("select mo.WorkedInterval from MoneyMan mo where SUBSTR(WorkDate,1,4)='" 
