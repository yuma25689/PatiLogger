...
html5jp.graph.circle.prototype.draw = function(items, inparams) {
	...
	/* 各項目のデフォルト色を定義 */
	var colors = ["24,41,206", "198,0,148", "214,0,0", "255,156,0", "33,156,0",
				 "33,41,107", "115,0,90", "132,0,0", "165,99,0", "24,123,0",
				 "40,41,53", "20,0,45", "64,0,0", "88,50,0", "128,128,128"
				 ];
	/* 円グラフを描写 */
	var rates = new Array();
	...
	for(var i=0; i<item_num; i++) {
		...
		if(n > 0) {
			...
			//radgrad.addColorStop(0, "rgb(" + rgb + ",1)");
			//radgrad.addColorStop(0.7, "rgb(" + rgb + ",0.85)");
			//radgrad.addColorStop(1, rgb);
			this.ctx.fillStyle = "rgb(" + rgb + ")";//radgrad;
			this.ctx.fill();
			/* 円弧の枠線 */
			...
			if(r <= 0.05) {
				...
				if(params.captionNum == true) {
					var numCap = n;
					// 2012/1/1 yuma25689 add start
					var numHour = parseInt( n / 60 );
					var numMinute = n % 60;
					numCap = ( numHour == 0 ? "" : numHour + "時間" ) + numMinute + "分";
					// 2012/1/1 yuma25689 add end
		...
		if(params.legend == true) {
			...
			//legend_radgrad.addColorStop(1, rgb );
			this.ctx.fillStyle = "rgb(" + rgb + ")"; //legend_radgrad;
			this.ctx.fill();
			/* */
...
html5jp.graph.circle.prototype._fold_items = function(items) {
	...
	if(len <= 15) { return items; }
	var n = 0;
	for( var i=14; i<len; i++ ) {
		n += items[i][1];
	}
	var newitems = items.slice(0, 15);
	newitems[14] = [this.params.otherCaption, n];
	return newitems;
};
...
html5jp.graph.circle.prototype._make_shadow = function (cpos) {
	...
	//	radgrad.addColorStop(0, 'rgb(0,0,0)');
		//radgrad.addColorStop(0.93, 'rgb(0,0,0)');
		//radgrad.addColorStop(1, 'rgb(0,0,0)');
	//}
	this.ctx.fillStyle = "#555555";//radgrad;
	this.ctx.fill();
};
